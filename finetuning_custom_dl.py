...
def create_hug_dataset(split_directory, split_train):
    ...
    print(f"N DATA TRAIN: {tot_len}")
    n_data_train = int((tot_len*split_train)/100)
    print(f"N DATA TRAIN: {n_data_train}, N DATA VAL: {tot_len-n_data_train}")
...
training_args = TrainingArguments(
            output_dir='./finetuning_jonata_ita_all/checkpoints',
            ...
            save_total_limit=1,
            logging_dir='./finetuning_jonata_ita_all/logs'
       )
...
try:
    ...
    print("#### Save model")
    trainer.save_model('./finetuning_jonata_ita_all/final')
except Exception as e:
    with open('./finetuning_jonata_ita_all/error.txt', 'w') as f:
        f.write(str(e))
    print(str(e))
